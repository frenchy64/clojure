...
(deftest syntax-quote-nil-optimization
  (testing "nil is self-evaluating in syntax-quote"
    ;; With optimization: `nil => nil
    ;; Without optimization: `nil => (quote nil)
    ;; This test verifies that nil syntax-quoting returns nil directly
    (is (nil? (eval '`nil)))
    ;; Verify the expansion is simple
    (binding [*ns* (the-ns 'clojure.test-clojure.reader)]
      (is (nil? '`nil)
          "Syntax-quoted nil should evaluate to nil at read-time"))))
